...
unwind <- function(model1, model2, use_model2_panel = "ALL", print_summary = TRUE,
  ...
  for (p in 1:base::length(main_m_groups)) {
    main_m_list[p] <- main_m_ini[
      main_m_ini$Group == main_m_groups[p],
      -1
    ] %>% base::list()
  }
  ...
  for (q in 1:base::length(main_m_groups)) {
    ...
    new_m1[, sub_var_name] <- main_var_for_sub * sub_m[
      ,
      "BASE"
    ]
    aggDateMelt <- function(x) {
      reshape2::melt(x, id.vars = c(
        "agg_period",
        "Date"
      ))
    }
    base::suppressWarnings(contribution <- dplyr::bind_rows(
      aggDateMelt(new_m1),
      aggDateMelt(new_m2[, -3])
    ))
    if (Base != "ModelKPI") {
      contribution$variable[contribution$variable == sub_var_name] <- "BASE"
    }
    contribution <- reshape2::dcast(contribution, agg_period +
                                      Date ~ variable, fun.aggregate = sum, value.var = "value")
    contribution <- aggDateMelt(contribution)
    base::names(contribution) <- base::c(
      "agg_period", "Date",
      "Categories", "value"
    )
    if (q == v & v == 1) {
      if (use_model2_panel != "ALL") {
        model2$variableTbl$Model <- model2$variableTbl$Model %>%
          base::paste0("(", use_model2_panel, ")", sep = "")
      }
      var_cat_tbl <- base::unique(dplyr::bind_rows(
        model1$variableTbl,
        model2$variableTbl
      ))
      ...
      var_order <- base::c(
        "agg_period", var_order[1:2],use_model2_panel,
        "BASE", var_order[-(1:2)]
      )
      if (Base != "ModelKPI") {
    ...
    summary <- contribution %>%
      reshape2::dcast(agg_period ~
                        Categories, fun.aggregate = sum, value.var = "value") %>%
      .[, var_order[-c(2, 3)]]
    rename_and_agg2 <- function(x) {
      reshape2::dcast(x, Date + agg_period ~ Categories,
    ...
    if (q == 1) {
      contribution_return <- base::cbind(
        Group = main_m_groups[q],
        contribution
      )
      summary_return <- base::cbind(
        Group = main_m_groups[q],
        summary
      )
    }
    else {
      contribution <- base::cbind(
        Group = main_m_groups[q],
        contribution
      )
      summary <- base::cbind(
      ...
      contribution_return <- base::rbind(
        contribution_return,
        contribution
      )
  ...
  subtotalmodel1 <- data.frame(matrix(NA, nrow = 1, ncol = (ncol(model1ttl) -
                                                              4)))
  ...
  return(list(
    ...
    varCont = var_tblPG, seankeyReadyTbl = sank
  ))
}
